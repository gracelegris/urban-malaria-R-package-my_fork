...
extract_raster_data <- function(state_name, state_shapefile, raster_paths) {
  ...
  tryCatch({
    ...
    wards <- state_shapefile
    wards <- wards[!sf::st_is_empty(wards), ]
    wards_vect <- terra::vect(wards)

    # Helper function
    extract_mean <- function(rasters) {
      mat <- terra::extract(rasters, wards_vect, fun = mean, na.rm = TRUE, ID = FALSE)
    ...
    if (!is.null(raster_paths$evi_path)) {
      ...
      wards$mean_EVI <- extract_mean(evi_stack)
    }
    ...
    if (!is.null(raster_paths$ndvi_path)) {
      ...
      wards$mean_NDVI <- extract_mean(ndvi_stack)
    }

    # Rainfall
    if (!is.null(raster_paths$rainfall_path)) {
      message("Extracting Rainfall for ", state_name)
      rain_files <- list.files(raster_paths$rainfall_path, pattern = ".tif$", full.names = TRUE)
      if (length(rain_files) == 0) stop("No rainfall files found.")
      rain_stack <- terra::rast(rain_files)
      wards$mean_rainfall <- extract_mean(rain_stack)
    }

    # Distance to Water Bodies
    if (!is.null(raster_paths$h2o_distance_path)) {
      message("Extracting Distance to Water Bodies for ", state_name)
      h2o_raster <- terra::rast(raster_paths$h2o_distance_path)
      wards$distance_to_water <- terra::extract(h2o_raster, wards_vect, fun = mean, na.rm = TRUE)[, 2]
    }

    # Relative Humidity
    if (!is.null(raster_paths$rh_2023) && !is.null(raster_paths$rh_2024)) {
      message("Extracting Relative Humidity for ", state_name)
      rh_stack <- terra::rast(c(raster_paths$rh_2023, raster_paths$rh_2024))
      wards$RH_mean <- extract_mean(rh_stack)
    }

    # Temperature
    if (!is.null(raster_paths$temp_2023) && !is.null(raster_paths$temp_2024)) {
      message("Extracting Temperature for ", state_name)
      temp_stack <- terra::rast(c(raster_paths$temp_2023, raster_paths$temp_2024))
      wards$temp_mean <- extract_mean(temp_stack)
    }

    # Housing Quality
    if (!is.null(raster_paths$housing_quality_path)) {
      message("Extracting Housing Quality for ", state_name)
      hq_raster <- terra::rast(raster_paths$housing_quality_path)
      wards$housing_quality <- terra::extract(hq_raster, wards_vect, fun = mean, na.rm = TRUE)[, 2]
    }

    # PfPR
    if (!is.null(raster_paths$pfpr_path)) {
      message("Extracting PfPR for ", state_name)
      pfpr_raster <- terra::rast(raster_paths$pfpr_path)
      wards$pfpr <- terra::extract(pfpr_raster, wards_vect, fun = mean, na.rm = TRUE)[, 2]
    }
    ...
    if (!is.null(raster_paths$lights_path)) {
      ...
      light_files <- list.files(raster_paths$lights_path, pattern = ".tif$", full.names = TRUE)
      if (length(light_files) == 0) stop("No night-time light files found.")
      light_stack <- terra::rast(light_files)
      wards$avgRAD <- extract_mean(light_stack)
    }

    # Floods
    if (!is.null(raster_paths$flood_path)) {
      message("Extracting Floods for ", state_name)
      flood_files <- list.files(raster_paths$flood_path, pattern = ".tif$", full.names = TRUE)
      if (length(flood_files) == 0) stop("No flood files found.")
      flood_stack <- terra::rast(flood_files)
      wards$flood <- extract_mean(flood_stack)
    }

    # NDWI
    if (!is.null(raster_paths$ndwi_path)) {
      message("Extracting NDWI for ", state_name)
      ndwi_raster <- terra::rast(raster_paths$ndwi_path)
      wards$NDWI <- terra::extract(ndwi_raster, wards_vect, fun = mean, na.rm = TRUE)[, 2]
    }

    # NDMI
    if (!is.null(raster_paths$ndmi_path)) {
      message("Extracting NDMI for ", state_name)
      ndmi_raster <- terra::rast(raster_paths$ndmi_path)
      wards$NDMI <- terra::extract(ndmi_raster, wards_vect, fun = mean, na.rm = TRUE)[, 2]
    }

    # Elevation
    if (!is.null(raster_paths$elevation_path)) {
      message("Extracting Elevation for ", state_name)
      elev_raster <- terra::rast(raster_paths$elevation_path)
      wards$elevation <- terra::extract(elev_raster, wards_vect, fun = mean, na.rm = TRUE)[, 2]
    }

    # Surface Soil Wetness
    if (!is.null(raster_paths$surface_soil_wetness_path)) {
      message("Extracting Surface Soil Wetness for ", state_name)
      wet_files <- list.files(raster_paths$surface_soil_wetness_path, pattern = ".tif$", full.names = TRUE)
      if (length(wet_files) == 0) stop("No soil wetness files found.")
      wet_stack <- terra::rast(wet_files)
      wards$mean_soil_wetness <- extract_mean(wet_stack)
    }
